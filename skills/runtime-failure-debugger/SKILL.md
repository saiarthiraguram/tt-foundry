...
```json
{
  ...
  "default_failure_type": "OOM | PCC drop | NaN PCC | L1 | Fake tensor | other",
...
Route by failure type identified in Phase 1:

- **Fake tensor / `TENSOR_DATA_NOT_ALLOCATED`** (PyTorch error: *"tensor has
  a non-zero number of elements, but its data is not allocated yet"*) →
  use **2C**, then proceed to Phase 3 → 3B → 3C → 3D (skip Phase 4/5
  unless 3D escalates).
- **OOM / L1 / generic runtime exception** → use **2A**, then Phase 3 →
  3B → 4 → 5.
- **PCC drop / NaN PCC**, or unidentified op from 2A → use **2B**, then
  Phase 3 → 3B → 4 → 5.
...
### 2C — Fake Tensor / `TENSOR_DATA_NOT_ALLOCATED`

Failure shape: `RuntimeError: The tensor has a non-zero number of elements,
but its data is not allocated yet.` Often surfaced through Dynamo /
`torch.compile` / `torch.export` while tracing a custom kernel that does
data-dependent shape arithmetic on a `FakeTensor`. The PyTorch error
message itself usually names the remedy: *"please wrap the custom kernel
into an opaque custom op."*

Identification steps (do not bisect for this failure type — the
traceback is reliable):

1. **Find the originating frame.** Scan the traceback for the lowest
   tt-xla–owned file. The common locations are:
   - `python_package/tt_torch/torch_overrides.py:*` — a TT override
     dispatching to a torch op (e.g. `torch.einsum`, `F.grid_sample`).
   - `python_package/tt_torch/utils.py:*` — Dynamo guard wrappers.
   - Any other `python_package/tt_torch/**.py` frame.
2. **Identify the underlying op** the override is calling. Read the
   `func()` body at the named line. Record the torch op name (e.g.
   `torch.einsum`, `F.scaled_dot_product_attention`).
3. **Capture input shape/dtype** at the call site. Add `loguru` info
   logs *inside the override*, around the `func(...)` call, printing
   `tensor.shape`, `tensor.dtype`, and `isinstance(t, torch._subclasses.FakeTensor)`
   for each input. Run the failing test → `claude_logs_<model_name>/op_trace.log`.
4. **Revert the loguru additions** once the trace is captured.
5. **Do NOT modify the override body** to skip the failing op or
   substitute a different implementation. The override is what is being
   debugged; the repair belongs in Phase 3C, not here.

The deliverable from 2C is the same as 2A: a named op + concrete input
shapes/dtypes. Proceed to Phase 3 to build the block sanity that calls
the same TT override (rule 6 — never reimplement).
...
## Phase 3C — Static-Shapes / Opaque-Kernel Repair (fake-tensor only)

Trigger: the minimal sanity from Phase 3B reproduces a
`TENSOR_DATA_NOT_ALLOCATED` failure. Skip this phase entirely for OOM,
PCC, NaN, L1, and generic runtime failures — those route directly to
Phase 4.

The PyTorch error message itself points at the fix: stop Dynamo from
tracing into a kernel whose output shape is data-dependent. Three
recipes, in order of escalation. **Apply at most one per iteration**
and re-run the minimal sanity after each.

### Recipe A — `torch._dynamo.mark_static` on the inputs (cheapest)

Pins selected input dims so Dynamo's meta path stops producing
unallocated FakeTensors with placeholder shape arithmetic. Edit the
**loader** (preferred — no graph break) rather than the model's
`forward`:

```python
# third_party/tt_forge_models/<model_name>/pytorch/loader.py
import torch._dynamo

def load_inputs(self):
    inputs = ...                            # existing
    torch._dynamo.mark_static(inputs, 0)    # batch dim
    torch._dynamo.mark_static(inputs, 1)    # seq dim, if relevant
    return inputs
```

Re-run the minimal sanity (Phase 3B). If it clears → Phase 3D. If it
still fails with the same `TENSOR_DATA_NOT_ALLOCATED` → Recipe B.

### Recipe B — Hardcode the shape at the call site upstream of the override

Use when `mark_static` is insufficient (typically because the dynamic
dim is computed *inside* the model, after Dynamo has already produced a
FakeTensor without a concrete size). Edit the **model code at the
caller of the failing TT override** so the tensor entering the override
has a fully concrete shape. Acceptable techniques:

- Explicit `.reshape(<literal_int>, <literal_int>, ...)` with integer
  literals derived from the model config (not from `.shape[...]`).
- `.contiguous()` followed by a no-op `.view(*list(t.shape))` where
  every element of the size list is a Python int (force concretization).
- Replace a `t.shape[i]` arithmetic expression with the literal int.

The edit lives in **model code** (`third_party/tt_forge_models/<model_name>/...`
or the upstream package). **Do not edit
`python_package/tt_torch/torch_overrides.py` here** — the override
stays generic. Re-run minimal sanity. If it clears → Phase 3D.
Otherwise → Recipe C.

### Recipe C — Wrap the override as a `torch.library.custom_op`

Last-resort and most invasive. Stop Dynamo from tracing through the
override entirely by registering it as an opaque op with a `register_fake`
that returns a meta tensor of the analytically-correct shape.

```python
# python_package/tt_torch/torch_overrides.py
import torch

@torch.library.custom_op("tt_xla::<op_name>", mutates_args=())
def <op_name>(<typed_args>) -> torch.Tensor:
    return <existing_override_body>(<args>)

@<op_name>.register_fake
def _<op_name>_fake(<typed_args>):
    # Compute the output shape analytically from the input shapes.
    # Must return a real-shaped meta tensor, not a FakeTensor with placeholders.
    return torch.empty(<derived_output_shape>, dtype=<args>[0].dtype, device=<args>[0].device)
```

Then re-dispatch the override path through `torch.ops.tt_xla.<op_name>`
in place of the previous direct `torch.<op>` call.

Re-run minimal sanity. If it clears → Phase 3D. If it **still** fails,
the underlying failure is not a fake-tensor issue anymore — write up
the path tried in `debug_report.md` and re-enter Phase 2 with the new
failure signature.

### Hard rules for Phase 3C

- **One recipe per iteration**, then re-run the minimal sanity. Do not
  stack repairs blindly.
- **Revert Recipe A's `mark_static` calls** before applying Recipe B or
  C — they may interact and mask the real fix being attempted.
- **Never edit the override body to bypass the failing op.** The
  override must continue to dispatch to the same torch op. Recipe C is
  the only sanctioned edit to `torch_overrides.py`, and it preserves
  behaviour while making the op opaque to Dynamo.
- **All edits live in model code or `tt_torch/torch_overrides.py`** —
  never in PyTorch internals, never in tt-mlir, never in the test
  runner.
- **Capture the diff.** Each attempted recipe gets a unique log file
  (`claude_logs_<model_name>/recipe_a.log`, `recipe_b.log`, `recipe_c.log`)
  *and* a saved patch
  (`claude_logs_<model_name>/recipe_<X>.patch`, produced via
  `git diff > <path>`) so the orchestrator and the human reviewer can
  see exactly what was attempted.

---

## Phase 3D — Verify on the Full Model (fake-tensor only)

A passing minimal sanity is necessary but not sufficient — fake-tensor
fixes commonly unmask a downstream failure. Re-run the original failing
test before declaring success.

### Step 1 — Re-run the original test

```bash
cd <tt_xla_repo>
TT_XLA_ARCH=<arch> timeout 600 python -m pytest \
  '<original_test_node_id>' \
  --runxfail -svv --tb=long -p no:cacheprovider \
  --json-report --json-report-file=claude_logs_<model_name>/post_repair_run.json \
  2>&1 | tee claude_logs_<model_name>/post_repair_run.log
```

`--runxfail` is **mandatory** — the model runner calls `pytest.xfail(...)`
in its finally path (`tests/runner/test_utils.py:748`) and would
otherwise hide the true outcome.

### Step 2 — Branch on the outcome

Read `post_repair_run.json` and inspect `tests[0].user_properties[*].tags`:

| Outcome | `failing_reason.name` | Meaning | Next action |
|---------|-----------------------|---------|-------------|
| `passed` AND `bringup_status: PASSED` | — | The repair is sufficient. | Stop. Emit `debug_report.md` with the recipe used (A/B/C) and the patch. Recommend orchestrator → CONFIG_UPDATE(PASSED). |
| `failed` | `TENSOR_DATA_NOT_ALLOCATED` (same description) | Same fake tensor — recipe insufficient. | Escalate within Phase 3C: A → B → C. If already on C, stop and escalate to a human via `debug_report.md`. |
| `failed` | differs (e.g. `INCORRECT_RESULT`, OOM, missing op) | Fake-tensor was masking a downstream failure. | Re-enter Phase 2 with the new failure signature, treating this as iteration N+1 of the debug session. Append to `debug_report.md`. |
| `failed` | `INCORRECT_RESULT` / PCC | Repair landed but PCC is now bad. | Switch to Phase 2B (PCC bisect) on the new failure. |

### Hard rules for Phase 3D

- **Always run with `--runxfail`** — without it, the runner's xfail
  block hides the real outcome and you will incorrectly declare success.
- **Never declare success from sanity alone.** Phase 3D's full-model
  re-run is the only acceptable success signal for a fake-tensor repair.
- **Cap recursion.** Stop after **3** rounds of `(Phase 3D reveals new
  failure) → Phase 2 → Phase 3 → Phase 3D`. After the 3rd round, write
  `debug_report.md` documenting every recipe attempted and every
  downstream failure surfaced, and hand off to a human.
...
- **Fake-tensor failures terminate at Phase 3D, not Phase 4/5.** When the failure type is `TENSOR_DATA_NOT_ALLOCATED`, the success signal is the full-model re-run in Phase 3D, not a TTNN/tt-metal repro. Skip Phase 4 and Phase 5 for this failure type unless Phase 3D surfaces a new, non-fake-tensor failure that needs them.
- **Fake-tensor repair edits stay above the framework boundary.** Phase 3C recipes live in model code (loader, model `forward`) or in `python_package/tt_torch/torch_overrides.py` as a `custom_op` registration — never in PyTorch internals, never in tt-mlir, never in the test runner. If a recipe seems to require editing outside this boundary, the diagnosis is wrong; re-enter Phase 2 with the new signal.
