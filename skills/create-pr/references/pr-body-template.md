...
Use these as a fallback if `.github/CODEOWNERS` isn't readable. Always re-check the live file when the skill runs.
...
Rules:
- Always include at least one global owner.
- Strip the PR author from the reviewer list.
- Cap at 15 reviewers (GitHub max).
